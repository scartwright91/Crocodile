...
#include "../include/LDtkLoader/Tile.hpp"
#include "../include/LDtkLoader/Layer.hpp"
#include "../include/LDtkLoader/Level.hpp"

using namespace ldtk;

const Tile Tile::None{nullptr, {-1, -1}, -1, 0};

Tile::Tile(const Layer *l, IntPoint pos, int tile_id, int flips) : layer(l),
...
auto Tile::getVertices() const -> std::array<Vertex, 4>
{
    ...
    verts[0].pos.x = static_cast<float>(pos.x);
    verts[1].pos.x = static_cast<float>(pos.x + cell_size);
    verts[0].pos.y = static_cast<float>(pos.y);
    verts[1].pos.y = static_cast<float>(pos.y);

    verts[3].pos.x = static_cast<float>(pos.x);
    verts[2].pos.x = static_cast<float>(pos.x + cell_size);
    verts[3].pos.y = static_cast<float>(pos.y + cell_size);
    verts[2].pos.y = static_cast<float>(pos.y + cell_size);
